...
-- Dosering: vrije tekst ("5 g", "2 capsules"); leeg = gewone habit, gevuld = supplement.
alter table public.custom_habits add column if not exists dose text not null default '';
-- Doses die nog in huis zijn. -1 = niet bijhouden, 0 = op; zie 0021.
alter table public.custom_habits add column if not exists stock_left integer not null default -1;
-- Zelfgekozen icoon en kleur; leeg = de app kiest. Kleur is een sleutel, geen hex.
