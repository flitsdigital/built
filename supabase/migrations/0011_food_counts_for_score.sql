-- Eten meetellen voor de Groei Score los van eten bijhouden.
--
-- `tracks_food` uit zette eten helemaal weg: geen eiwitkaart op het dashboard, geen
-- eiwitrij in de widget. Wie eten wél logt maar niet elke dag, wilde alleen dat een
-- vergeten dag geen 30 punten en zijn streak kostte. Dat is nu een eigen vlag.
-- Aan = gedrag van hiervoor, dus bestaande rijen merken er niets van.

alter table public.profiles add column if not exists food_counts_for_score boolean not null default true;

-- sync_push bijgewerkt: neemt food_counts_for_score mee.
