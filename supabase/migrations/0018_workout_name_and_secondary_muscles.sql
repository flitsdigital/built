-- Naam van de training, en secundaire spieren per oefening.
--
-- workout_name staat naast workout_note op day_habits: de notitie is proza ("voelde
-- zwaar"), de naam is een label ("Push A") dat het logboek als kop gebruikt. Leeg =
-- naamloos, dan valt het logboek terug op de datum zoals het altijd deed.
--
-- secondary_muscles is bewust géén tweede `muscle`-kolom: `muscle` bepaalt waar de
-- oefening in de bibliotheek en de spiersplit landt, en dat moet één spier blijven —
-- anders telt een oefening meerdere keren mee in het volume.
...
-- sync_push_v2 bijgewerkt: neemt beide kolommen mee. jsonb_populate_recordset leest de
-- rijen tegen het tabeltype, dus de nieuwe velden komen er vanzelf in zodra de kolom er is.
