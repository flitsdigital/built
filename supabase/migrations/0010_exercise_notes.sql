-- Notities per oefening als eigen veld.
--
-- Ze zaten als regels "Bench Press: voelde zwaar" in day_habits.note gepropt en
-- werden er op prefix weer uit geparsed — een relatie vermomd als tekst, die brak
-- op een oefeningsnaam met een dubbele punt. `note` blijft staan voor wat er al in
-- zit; de app leest daar nog uit terug als exercise_notes leeg is.

alter table public.day_habits add column if not exists exercise_notes jsonb not null default '{}'::jsonb;

-- sync_push bijgewerkt: neemt exercise_notes mee.
