-- Porties met eenheid. Een gelogd item bewaarde alleen het resultaat (eiwit/kcal/
-- koolhydraten/vet), niet hoevéél je nam. Daardoor was "250 g → 200 g" zelf vier macro's
-- herrekenen, en was drank in milliliters onmogelijk.
--
-- protein_entries.amount  hoeveelheid in `unit`; 0 = oudere invoer zonder portie
-- protein_entries.unit    'g' of 'ml'
-- food_products.unit      onthouden per product, zodat melk niet in grammen begint
-- food_products.last_amount  laatst gelogde portie = de volgende standaardwaarde
-- food_products.categories   OFF-categorieën (kommagescheiden), voor portie-suggesties
...
-- sync_push bijgewerkt: neemt portie, eenheid en categorieën mee.
