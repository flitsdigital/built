-- Een training is niet langer "alles wat je die dag deed".
--
-- Sets hingen alleen aan een datum, dus een tweede training op dezelfde dag schoof bij de
-- eerste in: één kaart in de historie, één naam, één notitie. `workout_id` geeft elke
-- sessie een eigen identiteit; sets zonder id zijn van vóór deze kolom en vallen in de app
-- terug op de dag, precies zoals ze er toen ook uitzagen.
--
-- Naam en notitie verhuizen mee naar per-sessie dictionaries. `workout_name` en
-- `workout_note` blijven staan: dat is wat de oude dagen nog dragen.
--
...
-- sync_push_v2 opnieuw, met de drie kolommen erin. Verder identiek aan 0019.
