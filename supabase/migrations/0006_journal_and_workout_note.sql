-- Journal (meerdere getimede notities per dag) + algemene workout-notitie op de dag/sessie.

alter table public.day_habits add column if not exists journal jsonb not null default '[]';
alter table public.day_habits add column if not exists workout_note text not null default '';

-- Bestaande dagnotitie eenmalig meenemen als eerste journal-entry (geen dataverlies).
update public.day_habits
set journal = jsonb_build_array(jsonb_build_object(
      'id', gen_random_uuid(),
      'text', note,
      'createdAt', to_jsonb(date)
    ))
where note <> '' and journal = '[]'::jsonb;

-- sync_push bijgewerkt: neemt journal + workout_note mee bij het herschrijven van day_habits.
