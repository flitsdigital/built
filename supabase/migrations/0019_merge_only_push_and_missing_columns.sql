-- Merge-only push (#42) + de zes kolommen die deze database nooit gekregen heeft.
--
-- Twee dingen in één migration, omdat ze samen één werkende staat opleveren:
--
-- 1. De kolommen uit 0008 en 0018 ontbraken op productie. `sync_push_v2` schrijft ze wél,
--    en een push is één transactie — dus liep élke push met een profiel erin stuk op
--    `column "tracks_food" of relation "profiles" does not exist`. Delta-pushes zonder
--    profielwijziging kwamen er nog doorheen, de wekelijkse volledige push nooit. Precies
--    het vangnet dat divergentie had moeten opruimen was dus al die tijd kapot.
--
-- 2. `sync_push_v2` had een vernietigende modus: met `full_replace = true` gold "dit
--    toestel is de waarheid" — alles wat de payload niet noemde werd getombstonet en de
--    conflictregel verviel. Naast de merge is dat overbodig, en precies zo groot als de
--    schade die het kan aanrichten. Een volledige push is voortaan een volledige *delta*:
--    alle rijen mee, zonder de "wat ik niet noem bestaat niet"-clausule.
--
-- Alles hieronder is herhaalbaar (`add column if not exists`, `create or replace`), dus
-- twee keer draaien kan geen kwaad.

-- MARK: - Ontbrekende kolommen (0008 en 0018), identiek aan daar

alter table public.profiles   add column if not exists tracks_food boolean not null default true;
alter table public.day_habits add column if not exists energy   int not null default 0;
alter table public.day_habits add column if not exists mood     int not null default 0;
alter table public.day_habits add column if not exists soreness int not null default 0;
alter table public.day_habits add column if not exists stress   int not null default 0;
alter table public.day_habits add column if not exists workout_name text not null default '';
alter table public.exercises  add column if not exists secondary_muscles jsonb not null default '[]'::jsonb;

-- `sync_pull` hoeft niet mee: die bouwt z'n antwoord met `to_jsonb(x)` over de hele rij,
-- dus nieuwe kolommen komen er vanzelf in.

-- MARK: - Push zonder vernietigende modus

-- `full_replace` blijft in de signatuur staan maar wordt genegeerd: een app-versie van
-- vóór deze migration stuurt 'm nog mee, en die mag geen "function does not exist" krijgen.
...
begin
  ...
  -- Er is geen uitzondering meer op die regel; zie de kop van deze migration.
