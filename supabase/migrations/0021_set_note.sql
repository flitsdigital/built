-- Een notitie hoort soms bij één set, niet bij de hele oefening van die dag.
--
-- "Derde set voelde de schouder" wil je later terugzien bij díe set. Wat er was, hing aan
-- de dag: `day_habits.exercise_notes` per oefening en `workout_notes` per sessie. Deze
-- kolom zet de notitie waar hij hoort.
--
-- `not null default ''` in plaats van een nullable tekstkolom: de app kent geen "geen
-- notitie" naast "lege notitie", en zo hoeft de pull geen onderscheid te maken tussen de
-- twee. Een toestel dat de kolom nog niet kent stuurt hem niet mee; die rijen komen als
-- NULL binnen en worden hier '' — precies wat ze al waren.
--
...
-- sync_push_v2 opnieuw, met `note` erin. Verder identiek aan 0020.
